...
for i in range(Ns):
    ...
    cartesian_task = pysot.oc.SE3VelTask("Cartesian", ocp.stage(Ns).model, dvariables.getVariable("dq"), "base_link")
    cartesian_task.setReferenceVelocity([0,1,0.5,0,1,1])
    cartesian_task.setWeight(1e3 * np.eye(6))
    minus.append(cartesian_task)

    ocp.stage(i).stack = pysot.AutoStack(minu + cartesian_task ) # TODO - check why it fails withoutit
